...
export default function ApprovalActions({ document, approvers, currentUserId }) {
    ...
    const isFinalApprover = approvers[approvers.length - 1].approver_id === currentUserId;
    ...
    const handleProcessApproval = async (newStatusInKorean) => {
        ...
        try {
            ...
            const approverId = myApprovalStep.approver_id;
            const newStatusForDB = newStatusInKorean === '승인' ? 'approved' : 'rejected';
            
            const result = await processApprovalAction({
                documentId,
                approverId,
                newStatus: newStatusForDB,
                comment,
                isFinalApprover,
            });
