...
export default function Sidebar({ isOpen, onClose }) {
    ...
    useEffect(() => {
        // 현재 경로에 따라 메뉴가 자동으로 열리도록 설정
        setIsWorkMenuOpen(pathname.startsWith('/work'));
        setIsDbMenuOpen(pathname.startsWith('/database')); // 새 DB 메뉴 경로
    }, [pathname]);
    ...
    const menuItems = [
      ...
      // --- DB 메뉴 추가 ---
      { name: 'DB', href: '/database', icon: DatabaseIcon, isDropdown: true, menuKey: 'db' },
                        ...
                        item.isDropdown ? (
                            <div key={item.name}>
                                <button onClick={() => toggleDropdown(item.menuKey)} className={`flex items-center justify-between w-full px-4 py-2 rounded-md text-sm font-medium transition-colors ${pathname.startsWith(item.href) ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white'}`}>
                                    ...
                                    <ChevronDownIcon className={`h-5 w-5 transition-transform duration-200 ${item.menuKey === 'work' ? (isWorkMenuOpen ? 'rotate-180' : '') : (isDbMenuOpen ? 'rotate-180' : '')}`} />
                                </button>
                                ...
                                {item.menuKey === 'db' && isDbMenuOpen && (
                                    <ul className="pt-2 pl-6 space-y-1">
                                        <li>
                                            <Link href="/database/tree-sales" className={`block px-3 py-2 text-sm rounded-md ${pathname === '/database/tree-sales' ? 'bg-gray-600 text-white' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`}>
                                                전국 조경수 DB
                                            </Link>
                                        </li>
                                        <li>
                                            <Link href="/database/construction-status" className={`block px-3 py-2 text-sm rounded-md ${pathname === '/database/construction-status' ? 'bg-gray-600 text-white' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`}>
                                                전국 공사 현황 DB
                                            </Link>
                                        </li>
                                    </ul>
