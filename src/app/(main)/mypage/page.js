...
function MyApprovalsWidget({ toReview, submitted, completed, referred }) {
    ...
    const getStatusChip = (status) => {
        const styles = { '진행중': 'bg-green-100 text-green-800', '대기': 'bg-yellow-100 text-yellow-800', '승인': 'bg-blue-100 text-blue-800', '반려': 'bg-red-100 text-red-800', };
    ...
    const renderList = (list) => {
        ...
        return ( <ul className="space-y-2">{list.map(doc => (<Link key={doc.id} href={`/approvals/${doc.id}`} className="block p-3 rounded-lg hover:bg-gray-100 border"><div className="flex justify-between items-center"><p className="font-medium text-gray-800 truncate">{doc.title}</p>{activeTab === 'toReview' && doc.approver_status ? getStatusChip(doc.approver_status) : getStatusChip(doc.status)}</div><p className="text-sm text-gray-500 mt-1">상신자: {doc.author?.full_name || '정보 없음'}</p></Link>))}</ul> );
    };
    return ( <div><div className="border-b border-gray-200 mb-4"><nav className="-mb-px flex space-x-6"><button onClick={() => setActiveTab('toReview')} className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'toReview' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>받은 결재 ({toReview.length})</button><button onClick={() => setActiveTab('submitted')} className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'submitted' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>상신한 결재 ({submitted.length})</button><button onClick={() => setActiveTab('completed')} className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'completed' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>완료된 결재 ({completed.length})</button><button onClick={() => setActiveTab('referred')} className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'referred' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>참조할 결재 ({referred.length})</button></nav></div><div className="overflow-y-auto max-h-64 pr-2">{activeTab === 'toReview' && renderList(toReview)}{activeTab === 'submitted' && renderList(submitted)}{activeTab === 'completed' && renderList(completed)}{activeTab === 'referred' && renderList(referred)}</div></div> );
}
...
function BusinessReportsWidget({ myReports, receivedReports }) {
    ...
    const renderList = (list, type) => {
        ...
        return ( <ul className="space-y-3">{list.map(log => (<Link href={`/approvals/${log.id}`} key={log.id} className="block p-3 rounded-lg hover:bg-gray-100 cursor-pointer border"><div className="flex justify-between items-center"><p className="font-semibold text-gray-800 truncate">{log.title}</p><span className="text-sm text-gray-500 whitespace-nowrap">{type === 'my' ? (log.recipient_name || '수신자 미지정') : log.author_name}</span></div><p className="text-xs text-gray-400 mt-1">{new Date(log.created_at).toLocaleDateString()}</p></Link>))}</ul> );
    };
    return ( <div><div className="border-b border-gray-200 mb-4"><nav className="-mb-px flex space-x-6"><button onClick={() => setActiveTab('received')} className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'received' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>보고받은 업무 ({receivedReports.length})</button><button onClick={() => setActiveTab('my')} className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'my' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>보고한 업무 ({myReports.length})</button></nav></div><div className="overflow-y-auto max-h-80 pr-2">{activeTab === 'received' ? renderList(receivedReports, 'received') : renderList(myReports, 'my')}</div></div> );
}
...
export default function MyPage() {
    ...
    const fetchData = useCallback(async (currentEmployee) => {
        if (!currentEmployee || !currentEmployee.id) { setLoadingSubData(false); return; }
        ...
        try {
            ...
            const fetchMySubmitted = async () => {
                ...
                if (error) console.error('상신한 결재 로드 오류:', error); return data || [];
            };
            const fetchToReview = async () => {
                const { data: approverEntries, error: entryError } = await supabase.from('approval_document_approvers').select('document_id, status').eq('approver_id', currentEmployee.id).eq('status', '대기');
                ...
                if (error) { console.error('받은 결재 문서 로드 오류:', error); return []; }
                return data?.map(doc => { const approverEntry = approverEntries.find(ae => ae.document_id === doc.id); return { ...doc, approver_status: approverEntry?.status }; }) || [];
            };
            const fetchCompleted = async () => { const { data, error } = await supabase.rpc('get_my_completed_approvals', { p_employee_id: currentEmployee.id }); if (error) console.error('완료된 결재 로드 오류:', error); return data || []; };
            const fetchReferredApprovals = async () => { const { data, error } = await supabase.rpc('get_my_referred_documents', { p_employee_id: currentEmployee.id }); if (error) console.error('참조할 결재 로드 오류:', error); return data || []; };
            const fetchMyReports = async () => { if (!workReportFormId) return []; const { data, error } = await supabase.from('approval_documents').select('id, title, created_at, form_fields').eq('author_id', currentEmployee.id).eq('form_id', workReportFormId); if (error) console.error('내 업무 보고서 로드 오류:', error); return (data || []).map(report => ({ ...report, recipient_name: report.form_fields?.수신자 || '지정 안됨' })); };
            const fetchReceivedReports = async () => { const { data, error } = await supabase.rpc('get_work_reports_for_user', { user_id_param: currentEmployee.id }); if (error) console.error('받은 업무 보고서 로드 오류:', error); return data || []; };
            const [submitted, toReview, completed, referred, myReports, receivedReports] = await Promise.all([ fetchMySubmitted(), fetchToReview(), fetchCompleted(), fetchReferredApprovals(), fetchMyReports(), fetchReceivedReports() ]);
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-full">
            <header className="mb-8 flex justify-between items-center">
                ...
                {/* ★★★ 개인정보 수정 페이지로 가는 링크 버튼 추가 ★★★ */}
                <Link href="/mypage/settings" className="px-4 py-2 bg-gray-200 text-gray-800 font-semibold rounded-lg hover:bg-gray-300 transition-colors">
                    개인정보 수정
                </Link>
            </header>
