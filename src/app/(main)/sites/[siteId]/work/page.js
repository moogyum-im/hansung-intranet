...
export default function DailyWorkPage() {
    ...
    const renderTable = (key) => {
        ...
        return (
            <div key={key} className="border border-slate-400 flex flex-col font-bold mb-1 w-full bg-white shadow-sm font-sans">
                <div className="bg-yellow-50 border-b border-slate-400 p-1 flex justify-between items-center font-black">
                    <span className="text-[10px] uppercase">▣ {config.title}</span>
                    {!isReadOnly && <button onClick={()=>setFormData(p=>({...p, [key]: [...(p[key]||[]), config.fields.reduce((a,f)=>({...a,[f]:''}),{})]}))} className="bg-white border border-slate-400 px-1 text-[8px] font-bold shadow-sm">+ 추가</button>}
                </div>
    ...
    return (
        <div className="h-screen bg-white font-bold font-sans flex flex-col overflow-hidden font-black italic-none">
            {selectedImage && (
            ...
            {isImportModalOpen && (
                <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 font-sans">
                    <div className="bg-white p-8 rounded-2xl max-w-sm w-full font-black border-4 border-slate-900 shadow-2xl">
                        <h3 className="text-xl mb-4 uppercase">데이터 불러오기</h3>
                        <input type="date" className="w-full p-3 border rounded-xl mb-4 font-black" onChange={e=>setImportDate(e.target.value)} />
                        <div className="flex gap-2"><button onClick={()=>setIsImportModalOpen(false)} className="flex-1 py-3 bg-slate-100 rounded-xl font-black">취소</button><button onClick={executeImport} className="flex-1 py-3 bg-black text-white rounded-xl font-black">불러오기</button></div>
                    </div>
                </div>
            )}

            <div className="flex justify-between items-center p-3 border-b border-slate-200 bg-white shrink-0 z-50 font-black">
                <div className="flex items-center gap-4">
                    <button onClick={() => router.back()} className="text-slate-600 font-black flex items-center gap-2 hover:bg-slate-50 p-2 rounded-lg transition-all font-sans italic-none"><ArrowLeft size={18}/> 현장목록</button>
                    {!isReadOnly && (
                        <div className="flex gap-1 border-l pl-4 border-slate-200 font-sans">
                            {Object.entries(FIELD_MAPS).map(([key, config]) => (
                                <button key={key} onClick={() => setVisibleSections(p => ({...p, [key]: !visibleSections[key]}))} className={`px-2.5 py-1.5 text-[9px] rounded-lg transition-all font-black ${visibleSections[key] ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-300 border border-slate-100'}`}>{config.title.split('(')[0]}</button>
                            ))}
                        </div>
                    )}
                </div>
                <div className="flex gap-3 items-center">
                    <div className="flex items-center gap-4 bg-slate-50 px-3 py-1.5 rounded-xl mr-4 border border-slate-200 font-sans">
                    ...
                    {isReadOnly ? (
                        <div className="flex gap-2">
                            <button onClick={handleDelete} className="bg-red-50 text-red-600 border border-red-200 px-6 py-2 rounded-xl font-black text-[11px] hover:bg-red-100 flex items-center gap-2 transition-all font-sans italic-none"><Trash2 size={14}/> 삭제</button>
                            <button onClick={() => setView('write')} className="bg-slate-900 text-white px-8 py-2 rounded-xl font-black text-[11px] shadow-sm flex items-center gap-2 hover:bg-slate-800 transition-all font-sans italic-none"><Edit3 size={14}/> 정보 수정</button>
                        </div>
                    ) : (
                        <div className="flex gap-2">
                             <button onClick={() => setIsImportModalOpen(true)} className="bg-amber-500 text-white px-5 py-2 rounded-xl font-black text-[11px] shadow-sm hover:bg-amber-600 flex items-center gap-2 transition-all font-sans italic-none"><RefreshCw size={14}/> 불러오기</button>
                             <button onClick={handleSave} disabled={isSaving} className="bg-blue-600 text-white px-8 py-2 rounded-xl font-black text-[11px] shadow-sm hover:bg-blue-700 transition-all flex items-center gap-2 font-sans italic-none">{isSaving ? <Loader2 className="animate-spin" size={14}/> : <Save size={14}/>} 일보 저장</button>
                        </div>
                    )}
                </div>
            </div>

            <div className="flex-1 overflow-auto p-6 bg-[#F8FAFC] font-black italic-none">
                    ...
                    <div className="max-w-[1600px] w-full bg-white p-8 pt-4 border border-slate-200 shadow-sm font-black italic-none">
                        ...
                        <div className="flex justify-between items-end gap-8 mb-4">
                            <div className="flex flex-col gap-2 flex-1 font-black italic-none">
                                <h1 className="text-5xl font-black uppercase tracking-[0.6em] text-left mt-[-30px] font-sans">작 업 일 보</h1>
                                <div className="flex gap-2 flex-wrap items-end font-sans italic-none font-black">
                                    <div className="flex border border-slate-400 text-[9px] font-black h-8 shrink-0 shadow-sm items-center px-3 bg-slate-50 uppercase">총 도급액: {formatNumber(formData?.total_contract_amount)}원</div>
                                    <div className="flex border border-slate-400 text-[9px] font-black h-8 shrink-0 shadow-sm items-center px-3 text-blue-700 font-sans">금일 사용: {formatNumber(totalTodaySpend)}원</div>
                                    <div className="flex border border-slate-400 text-[11px] bg-red-50 font-black h-8 shrink-0 shadow-sm items-center px-4 text-red-600 font-sans">누적 집행: {formatNumber(totalAccumSpend)}원 ({spendRate}%)</div>
                                    <div className="flex border border-slate-400 text-[11px] bg-blue-50 font-black h-8 shrink-0 shadow-sm items-center px-4 text-blue-800 font-sans gap-4">
                            ...
                            <div className="flex flex-col gap-0 w-[350px] font-black shrink-0 border border-slate-400 shadow-sm bg-white font-sans italic-none">
                                <div className="flex border-b border-slate-400 h-8 font-black italic-none">
                                    <div className="bg-slate-50 px-2 flex items-center border-r border-slate-400 text-[9px] w-20 font-black font-sans">현장명</div>
                                    <div className="px-2 flex items-center text-[10px] font-black font-sans overflow-hidden whitespace-nowrap flex-1">{siteData?.name}</div>
                                    <div className="bg-slate-50 px-2 flex items-center border-l border-r border-slate-400 text-[9px] w-20 font-black font-sans">작업 일시</div>
                                    <input type="date" className="px-2 text-[10px] outline-none bg-transparent font-sans font-black w-28" value={formData?.report_date || ''} readOnly={isReadOnly} onChange={e=>setFormData({...formData, report_date: e.target.value})} />
                                </div>
                        ...
                        <div className="grid grid-cols-12 gap-4 items-stretch mb-4 font-black italic-none">
                            <div className="col-span-8 grid grid-cols-2 border border-slate-400 h-[220px] shadow-inner bg-slate-50 font-sans italic-none font-black">
                                <div className="border-r border-slate-400 flex flex-col font-black italic-none"><div className="bg-slate-50 p-1.5 text-center border-b border-slate-400 text-[10px] uppercase font-sans tracking-widest">전일 작업 요약</div><textarea className="w-full flex-1 p-4 text-[13px] outline-none font-bold resize-none bg-transparent text-center leading-relaxed font-sans" value={formData?.prev_work} readOnly /></div>
                                <div className="flex flex-col font-black italic-none"><div className="bg-slate-50 p-1.5 text-center border-b border-slate-400 text-[10px] uppercase font-sans tracking-widest">금일 작업 요약</div><textarea className="w-full flex-1 p-4 text-[13px] outline-none font-bold resize-none bg-transparent text-center leading-relaxed font-black" value={formData?.today_work} readOnly={isReadOnly} onChange={e=>setFormData({...formData, today_work: e.target.value})} /></div>
                            </div>
                            <div className="col-span-4 border border-slate-400 shadow-sm overflow-hidden font-sans flex flex-col h-full bg-white italic-none">
                                <div className="bg-slate-800 text-white p-1.5 text-center text-[10px] uppercase font-sans font-black">실시간 정산 내역 합계</div>
                                <table className="w-full text-[9px] border-collapse font-sans flex-1">
                                    <thead className="bg-slate-50 border-b border-slate-400 font-bold font-sans italic-none"><tr><th className="p-1 border-r border-slate-400 text-center uppercase">항목</th><th className="border-r border-slate-400 text-center uppercase">전일</th><th className="border-r border-slate-400 text-center text-blue-600 font-bold uppercase">금일</th><th className="text-center text-red-600 uppercase font-bold">누계</th></tr></thead>
                        ...
                        <div className="grid grid-cols-3 gap-3 items-start font-black italic-none font-sans">
                            <div className="space-y-3">{renderTable('labor_costs')}</div>
                            <div className="space-y-3">{renderTable('material_costs')}{renderTable('equipment_costs')}{renderTable('tree_costs')}</div>
                            <div className="space-y-3">{renderTable('transport_costs')}{renderTable('subcontract_costs')}{renderTable('etc_costs')}</div>
                        </div>

                        <div className="pt-8 space-y-6 font-black border-t-2 border-slate-400 mt-4 italic-none">
                            <h4 className="text-lg font-black flex items-center gap-3 font-sans font-black italic-none"><ImageIcon size={24} className="text-slate-900"/> 시공 사진 대지</h4>
                            <div className="grid grid-cols-4 gap-4 font-black font-sans">
                                {Array.from({ length: maxPhotoRows }).map((_, idx) => (
                                    <div key={idx} className="grid grid-cols-2 gap-2 h-[260px] bg-white">
                                        {['tomorrow', 'today'].map(type => {
                                            const photo = (type === 'today' ? todayPhotos : tomorrowPhotos)[idx];
                                            const imgSrc = photo?.preview || photo?.url;
                                            return (
                                                <div key={type} className="flex flex-col gap-2 h-full italic-none">
                                                    <div className={`text-[8px] text-center font-black py-1.5 rounded-lg font-sans ${type==='today'?'bg-blue-100 text-blue-700':'bg-slate-100 text-slate-400'}`}>{type==='today'?'금일 현황':'전일 현황'}</div>
                                                    <div className="flex-1 bg-slate-50 border border-slate-300 rounded-[1.5rem] flex items-center justify-center cursor-zoom-in overflow-hidden relative shadow-inner italic-none font-black" onClick={() => imgSrc && setSelectedImage(imgSrc)}>
                                                        {imgSrc ? <img src={imgSrc} className="w-full h-full object-cover" alt="사진" /> : <Camera size={28} className="opacity-10" />}
                                                        {!isReadOnly && <input type="file" className="absolute inset-0 opacity-0 cursor-pointer italic-none font-black" onChange={(e) => handlePhotoUpload(e, type, idx)} />}
                                                    </div>
                                                    <input className="border-b border-slate-300 p-1 text-[10px] text-center bg-transparent font-black font-sans focus:border-slate-900 outline-none transition-all italic-none" placeholder="기록 입력" value={photo?.description || ''} readOnly={isReadOnly} onChange={e => { const n = (type === 'today' ? [...todayPhotos] : [...tomorrowPhotos]); if(!n[idx]) n[idx]={id:uuidv4()}; n[idx].description = e.target.value; (type === 'today' ? setTodayPhotos : setTomorrowPhotos)(n); }} />
                                                </div>
                                            );
                                        })}
                                    </div>
