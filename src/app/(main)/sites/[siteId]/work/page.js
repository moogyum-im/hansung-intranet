    ...
    useEffect(() => {
        const load = async () => {
            ...
            if (reportId) {
                ...
                const buildPhotoArray = (type) => {
                    const filtered = rep.photos?.filter(p => p.timeType === type) || [];
                    if (filtered.every(p => typeof p.rowIndex !== 'number')) return filtered;
                    const arr = [];
                    filtered.forEach(p => { arr[typeof p.rowIndex === 'number' ? p.rowIndex : arr.length] = p; });
                    return arr;
                };
                setTodayPhotos(buildPhotoArray('today'));
                setTomorrowPhotos(buildPhotoArray('tomorrow'));
            } else {
                const initialData = {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const photosToUpload = [
                ...todayPhotos.map((p, idx) => (p && (p.file || p.url)) ? { ...p, rowIndex: idx } : null),
                ...tomorrowPhotos.map((p, idx) => (p && (p.file || p.url)) ? { ...p, rowIndex: idx } : null),
            ].filter(Boolean);
            ...
            for (let i = 0; i < photosToUpload.length; i++) {
                ...
                if (p.url) {
                    uploadedPhotos.push({ id: p.id, url: p.url, timeType: p.timeType, description: p.description, rowIndex: p.rowIndex });
                } else {
                    ...
                    uploadedPhotos.push({ id: p.id, url: data.publicUrl, timeType: p.timeType, description: p.description, rowIndex: p.rowIndex });
                }
...
await supabase.from('construction_sites').update({
    ...
    const handleRemoveSinglePhoto = (type, idx) => {
        if (!confirm(`${type === 'today' ? '금일' : '전일'} 사진을 삭제하시겠습니까?`)) return;
        const setter = type === 'today' ? setTodayPhotos : setTomorrowPhotos;
        setter(prev => {
            const n = [...prev];
            n[idx] = undefined;
            return n;
                                                ...
                                                return (
                                                    <div key={type} className={`flex flex-col ${typeIdx === 0 ? 'border-r border-slate-100' : ''}`}>
                                                        ...
                                                        <div className={`flex-1 flex items-center justify-center overflow-hidden relative min-h-[160px]
                                                            ...
                                                            {imgSrc && !isReadOnly && (
                                                                <button
                                                                    onClick={(e) => { e.stopPropagation(); handleRemoveSinglePhoto(type, idx); }}
                                                                    className="absolute top-1.5 right-1.5 flex items-center justify-center w-5 h-5 bg-black/60 text-white opacity-0 group-hover:opacity-100 hover:bg-red-600 transition-all"
                                                                    title={`${isToday ? '금일' : '전일'} 사진 삭제`}>
                                                                    <Trash2 size={10} />
                                                                </button>
                                                            )}
                                                            {!isReadOnly && !imgSrc && <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => handlePhotoUpload(e, type, idx)} />}
                                                        </div>
                                                        <input
