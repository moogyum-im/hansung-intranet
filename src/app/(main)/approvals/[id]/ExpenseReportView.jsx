// 파일 경로: src/app/(main)/approvals/[id]/ExpenseReportView.jsx
'use client';
...
// 지출결의서 전용 상세 보기 컴포넌트
const ExpenseReportView = ({ doc, profile, approvers, currentUserId }) => {
                            ...
                            <table className="w-full text-sm border-collapse border border-gray-300 mb-8">
                             ...
                             <div className="mb-8 border border-gray-300">
                                ...
                                <table className="w-full text-sm border-collapse"><tbody>
                                    <tr><th className="p-2 bg-gray-100 font-bold w-1/4 text-left border-r border-b">지출일자</th><td className="p-2 w-1/4 border-r border-b">{formData.expenseDate}</td><th className="p-2 bg-gray-100 font-bold w-1/4 text-left border-r border-b">금액</th><td className="p-2 w-1/4 border-b">{Number(formData.amount).toLocaleString()} 원</td></tr>
                                    <tr><th className="p-2 bg-gray-100 font-bold text-left border-r">계정 과목</th><td className="p-2 border-r">{formData.accountType}</td><th className="p-2 bg-gray-100 font-bold text-left border-r">결제 수단</th><td className="p-2">{formData.paymentMethod}</td></tr>
                                </tbody></table>
                            </div>
                            <div><h2 className="text-lg font-bold mb-2">상세 내역 (적요)</h2><div className="p-4 border rounded-md min-h-[100px]"><p>{formData.description}</p></div></div>
                        </div>
