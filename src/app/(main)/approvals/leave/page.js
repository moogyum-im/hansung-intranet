...
export default function LeaveRequestPage() {
    ...
    useEffect(() => {
        ...
        setDocumentNumber(`DOC-${year}${month}${day}-${timestamp}`);
    }, []);
    ...
    useEffect(() => {
        ...
        if (!employeeLoading && employee) {
            ...
            if (employee?.team_leader_id) {
                setApproverId(employee.team_leader_id);
            }
        }
            ...
            <div className="flex-1">
                <div className="bg-white p-10 rounded-xl shadow-lg border">
                    ...
                    <div className="mb-8 border border-gray-300"><h2 className="p-2 bg-gray-100 font-bold border-b">신청 정보</h2><div className="p-4">
                        <div className="grid grid-cols-2 gap-4"><div><label className="block text-gray-700 font-bold mb-2 text-sm">휴가 종류</label><select name="leaveType" value={formData.leaveType} onChange={handleChange} className="w-full p-2 border rounded-md text-sm"><option>연차</option><option>오전 반차</option><option>오후 반차</option><option>월차</option><option>결근</option><option>기타</option></select></div><div><label className="block text-gray-700 font-bold mb-2 text-sm">휴가 일수</label><input type="text" name="duration" value={duration} readOnly className="w-full p-2 border rounded-md text-sm bg-gray-100" /></div></div>
            ...
            <div className="w-96 p-8">
                <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-lg border space-y-6 sticky top-8">
                    <div className="border-b pb-4">
                        <h2 className="text-lg font-bold mb-4">결재자</h2>
                        <select 
                            value={approverId} 
