...
import { X, Plus, Loader2, Database, CheckCircle, Users, ImageIcon, Info, Bold, Palette, Highlighter, Save } from 'lucide-react';
...
function InternalApprovalPage() {
    ...
    // (다른 결재자가 결재 진행 중인 문서를 수정할 수 있게 되면서, 편집자 본인 정보로 기안자가 바뀌어 보이는 것을 방지)
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            ...
            if (!draftId) router.replace(`/approvals/internal?draftId=${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const submissionData = {
                title: `내부결재: ${formData.title} (${requesterInfo.full_name})`,
    ...
    if (employeeLoading || editLoading) return <div className="p-10 text-black font-black text-xs h-screen flex items-center justify-center font-sans animate-pulse font-black uppercase">HANSUNG ERP SYNCING...</div>;
            ...
            <div className="w-full max-w-[1100px] mb-4 flex justify-between items-center no-print px-2 font-black">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={handleSubmit} disabled={loading || isUploading} className="flex items-center gap-2 px-6 py-2 bg-black text-white border border-black hover:bg-slate-800 text-[11px] shadow-lg transition-all active:scale-95 font-black">
                        {loading ? <Loader2 size={14} className="animate-spin font-black" /> : isUploading ? "업로드 중..." : editId ? <><CheckCircle size={14} /> 수정 저장</> : <><CheckCircle size={14} /> 내부결재 상신</>}
                    </button>
                </div>
            </div>
                ...
                <div className="lg:col-span-8 bg-white border border-black p-10 sm:p-14 shadow-sm relative text-black font-black font-black">
                    ...
                    <div className="space-y-10 text-black font-black font-black">
                        ...
                        <div className="pt-16 text-center space-y-6 font-black text-black font-black font-black">
                            ...
                            <p className="text-xl font-black uppercase tracking-widest font-black font-black">기안자: {requesterInfo.full_name} (인)</p>
                        </div>
                    </div>
                </div>
